...
## Mobile Service > IAP > Google 콘솔 가이드 { #mobile-service-iap-google-console-guide }
...
### 3. 마켓 연동 검증 생략 { #skip-market-integration-verification }
...
## 연동을 위한 두 가지 인증 방식 제공 { #two-authentication-methods-for-integration }
...
## Google Cloud 프로젝트 설정 { #set-up-google-cloud-project }
...
### 2. 프로젝트에서 사용할 API 추가 { #add-apis-to-use-in-your-project }
...
### 3. Google Cloud Console 메뉴 노출 { #show-the-google-cloud-console-menu }
...
## SUPERVISOR 연동 방식 설정 { #set-up-supervisor-integration }
...
### 1. OAuth 동의 화면 구성 { #configure-the-oauth-consent-screen }
...
### 2. Google Cloud 클라이언트 ID 생성 { #create-a-google-cloud-client-id }
...
### 3. OAuth 클라이언트로 Refresh token 생성 { #create-a-refresh-token-with-oauth-client }
...
### 4. NHN Cloud IAP 앱에서 클라이언트 정보 설정 { #set-up-client-information-in-nhn-cloud-iap-app }
...
## SERVICE_ACCOUNT 연동 방식 설정 { #setting-up-the-serviceaccount-integration-method }
...
### 1. Google Cloud 서비스 계정 생성 { #create-a-google-cloud-services-account }
...
### 2. Google Cloud 서비스 계정의 키 생성 { #generate-a-key-for-your-google-cloud-services-account }
...
### 3. Google Play Console에 서비스 계정 등록 { #register-a-service-account-with-google-play-console }
...
### 4. NHN Cloud IAP 앱에서 서비스 계정 설정 { #set-up-a-service-account-in-the-nhn-cloud-iap-app }
...
## 실시간 구독 상태 수신을 위한 Google 알림 설정 { #set-up-google-notifications-to-receive-real-time-subscription-status }
...
### 1. Google Cloud 알림 주제 생성 { #create-a-google-cloud-notification-topic }
...
### 2. 주제에 게시할 구독 설정 { #set-up-subscriptions-to-publish-to-the-topic }
...
### 3. Google Play Console에 구독 주제 등록 { #register-a-subscription-topic-in-google-play-console }
